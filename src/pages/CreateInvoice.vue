                ...
                <div class="card-body p-4">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-5">
                            ...
                            <select class="form-select form-select-lg" v-model="invoice.customer"
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="date" class="form-control form-control-lg" v-model="invoice.date" />
                        </div>
                        <div class="col-md-3">
                            <label class="form-label fw-semibold">Discount %</label>
                            <input type="number" class="form-control form-control-lg"
                                v-model.number="invoice.discountPercent" min="0" max="100" step="0.1"
                        ...
                        <div class="card-body p-0">
                            <div class="table-responsive">
                                <table class="table table-borderless mb-0 align-middle">
                                    ...
                                    <tbody>
                                        <tr v-for="(row, idx) in rows" :key="idx" class="border-bottom">
                                            <td class="ps-3 py-3">
                                                <select class="form-select" v-model="row.item"
                                            ...
                                            <td class="py-3">
                                                <input type="number" class="form-control" min="1"
                                                    v-model.number="row.qty" @input="onQtyOrPriceChange(idx)" />
                                            </td>
                                            <td class="py-3">
                                                <input type="number" class="form-control" min="0" step="0.01"
                                                    v-model.number="row.price" @input="onQtyOrPriceChange(idx)" />
                                            </td>
                                            <td class="py-3">
                                                <div class="form-control bg-white border-0 fw-bold text-success">
                                                    {{ formatMoney(row.total) }}
                                                </div>
                                            </td>
                                            <td class="py-3">
                                                <button type="button" class="btn btn-sm btn-outline-danger"
                                                    @click="removeRow(idx)" :disabled="rows.length === 1">
                            ...
                            <div class="card border-0 bg-light">
                                <div class="card-body">
                                    ...
                                    <div class="d-flex justify-content-between mb-2" v-if="invoice.discountPercent > 0">
                                        <span class="text-muted">Discount ({{ invoice.discountPercent }}%):</span>
                                        <span class="text-danger fw-semibold">- {{ formatMoney(discountAmount) }}</span>
                                    </div>
                                    <hr>
                    ...
                    <div class="d-flex justify-content-end gap-3 mt-4 border-top pt-4">
                        <button type="button" class="btn btn-secondary px-4 btn-lg"
                            @click="$router.push('/dashboard/invoices')">
                            Cancel
                        </button>
                        <button type="button" class="btn btn-primary px-5 btn-lg" @click="handleSubmit">
